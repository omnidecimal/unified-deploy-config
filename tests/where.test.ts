...
import type { EnvironmentComponentResult } from '../src/types/index.js';
...
describe('where command', () => {
  ...
  function runWhere(configFile: string, component: string, outputFormat: 'json' | 'list' = 'json'): EnvironmentComponentResult[] | string[] {
    const result = execSync(
      `node ${cliPath} where --config "${configFile}" --component ${component} --output ${outputFormat}`,
      { encoding: 'utf8' }
    );
    return outputFormat === 'json' ? JSON.parse(result) as EnvironmentComponentResult[] : result.trim().split('\n');
  }
  ...
  describe('JSON output', () => {
    test('should find component valid at environment level', () => {
      ...
      const result = runWhere(configFile, 'mycomponent') as EnvironmentComponentResult[];
    ...
    test('should show component not found', () => {
      ...
      const result = runWhere(configFile, 'nonexistent') as EnvironmentComponentResult[];
  ...
  describe('list output', () => {
    test('should list only valid environments and regions', () => {
      ...
      const result = runWhere(configFile, 'mycomponent', 'list') as string[];
