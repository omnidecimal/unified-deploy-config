...
const AwsRegionToShortCode = Object.fromEntries(
    Object.entries(AwsRegionMapping).map(([short, full]) => [full, short])
);

/**
 * Get the short region code from a full region name.
 * @param {string} fullRegion - Full region name (e.g., 'us-west-2')
 * @returns {string} Short code (e.g., 'usw2') or the input if not found
 */
function getRegionShortCode(fullRegion) {
    return AwsRegionToShortCode[fullRegion] || fullRegion;
}

/**
 * Get the full region name from a short code.
 * @param {string} shortCode - Short region code (e.g., 'usw2')
 * @returns {string} Full name (e.g., 'us-west-2') or the input if not found
 */
function getRegionFullName(shortCode) {
    return AwsRegionMapping[shortCode] || shortCode;
}

/**
 * Parse a target ID into environment and region components.
 * Target format: environmentname[-regionshortcode] (e.g., 'dev-usw2')
 * @param {string} target - Target ID to parse
 * @returns {{ env: string, region: string|undefined }} Parsed environment and region
 */
function parseTarget(target) {
    // Try to find a region short code at the end of the target
    for (const regionCode of Object.keys(AwsRegionMapping)) {
...
module.exports.getRegionShortCode = getRegionShortCode;
module.exports.getRegionFullName = getRegionFullName;
